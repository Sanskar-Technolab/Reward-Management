...
function delete_attachments(frm) {
  // Show confirmation dialog
  frappe.confirm(
    "Are you sure you want to delete all attachments for this Product QR?",
    function() {
      // User confirmed, proceed with deletion
      frappe.call({
        method: 'reward_management.reward_management.doctype.product_qr.product_qr.delete_product_qr_attachments',
        args: {
          docname: frm.doc.name,  // Use current document name
          enqueue: true
        },
        callback: function(response) {
          if (response.message.status === 'queued') {
            frappe.show_alert({
              message: __('Deletion started in background...'),
              indicator: 'blue'
            });
            
            // Listen for completion
            const handler = frappe.realtime.on('attachment_deletion_complete', (data) => {
              if (data.docname === frm.doc.name) {
                frappe.show_alert({
                  message: data.message,
                  indicator: data.status === 'success' ? 'green' : 'red'
                });
                frappe.realtime.off('attachment_deletion_complete', handler);
                
                // Refresh the form if deletion was successful
        ...
        freeze: true,
        freeze_message: __('Requesting attachment deletion...')
      });
    },
    function() {
      // User cancelled
