...
#         quantity = int(quantity)
#         now_dt = now()
#         timestamp = datetime.strptime(now_dt, "%Y-%m-%d %H:%M:%S.%f").timestamp()
#         current_date = format_datetime(now_dt, "yyyy-MM-dd")
#         current_time = format_datetime(now_dt, "HH:mm:ss")

#         product_doc = frappe.get_doc("Product", product_name)
#         reward_points = product_doc.get("reward_points", 0)

#         product_qr_doc = get_or_create_product_qr(product_name, quantity)

#         existing_ids = set(frappe.get_all("QR Data", pluck="product_qr_id"))
#         generated_ids = set()
#         qr_data_list = []

#         for i in range(quantity):
#             product_qr_id = generate_unique_id(product_name, product_qr_doc.name, i, timestamp, existing_ids, generated_ids)

#             qr_content = f"{product_qr_doc.name}_{product_name}_{product_qr_id}"
#             file_url = generate_qr_image(qr_content, product_qr_id)

#             # Generate custom name using getseries
#             prefix = f"{product_qr_id}-"
#             series_number = frappe.model.naming.getseries(prefix, 5)
#             new_name = f"{prefix}{int(series_number):05d}"

#             qr_data_list.append({
#                 "name": new_name,
#                 "product_qr": product_qr_doc.name,
#                 "product_qr_id": str(product_qr_id),
#                 "product_table_name": product_name,
#                 "generated_date": current_date,
#                 "generated_time": current_time,
#                 "points": reward_points,
#                 "qr_code_image": file_url,
#                 "product_qr_name": qr_content,
#                 "creation": now_dt,
#                 "modified": now_dt,
...
#         # Bulk insert with custom names
#         frappe.db.bulk_insert("QR Data", fields=list(qr_data_list[0].keys()), values=[list(row.values()) for row in qr_data_list])
...
# def get_or_create_product_qr(product_name, quantity):
#     existing = frappe.get_all("Product QR", filters={"product_name": product_name}, fields=["name"], order_by="creation desc", limit=1)
#     if existing:
#         doc = frappe.get_doc("Product QR", existing[0].name)
#         doc.quantity += quantity
#         doc.save(ignore_permissions=True)
#         return doc
#     else:
#         doc = frappe.new_doc("Product QR")
#         doc.update({
...
# def generate_unique_id(product_name, product_qr_name, index, timestamp, existing_ids, generated_ids, max_attempts=10):
#     for attempt in range(1, max_attempts + 1):
#         hash_input = f"{timestamp}_{product_name}_{product_qr_name}_{index}_{attempt}"
#         qr_id = int(hashlib.md5(hash_input.encode()).hexdigest(), 16) % 100000000
#         qr_id_str = str(qr_id)

#         if not qr_id_str.startswith("0") and qr_id_str not in existing_ids and qr_id_str not in generated_ids:
#             existing_ids.add(qr_id_str)
#             generated_ids.add(qr_id_str)
#             return qr_id

#     frappe.throw(_("Failed to generate unique QR ID after {0} attempts").format(max_attempts))




# def generate_qr_image(content, file_id):
#     try:
#         # Generate basic QR code image (default size)
#         qr = qrcode.QRCode(
#             version=1,
...
#         # Convert mm to pixels (100x75 mm at 300 DPI)
#         width_px = int((100 / 25.4) * 300)  # ≈ 1181 px
#         height_px = int((75 / 25.4) * 300)  # ≈ 886 px

#         # Resize image
#         img = img.resize((width_px, height_px), resample=Image.LANCZOS)
...
#         # File name and URL setup
#         file_name = f"{file_id}.png"

#         # Create the file document in Frappe
#         file_doc = frappe.get_doc({
#             "doctype": "File",
#             "file_name": file_name,
#             "attached_to_doctype": "QR Data",
#             "attached_to_name": content,
#             "is_private": 0,
#         })

#         # Save the file document first
#         file_doc.insert(ignore_permissions=True)

#         # Set the content of the file and save it again
#         file_doc.set_content(img_bytes.read())
#         file_doc.save(ignore_permissions=True)

#         return file_doc.file_url

#     except Exception as e:
#         frappe.log_error(f"QR image generation failed: {e}", "generate_qr_image")
#         return ""

    
# def generate_qr_image(content, file_id):
#     try:
#         qr_api_url = f"https://api.qrserver.com/v1/create-qr-code/?data={content}&size=100x75"
#         response = requests.get(qr_api_url)
