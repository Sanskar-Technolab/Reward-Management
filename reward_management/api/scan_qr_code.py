...
# scann qr code from the qr code image-----------
@frappe.whitelist()
def scan_qr_code(decode_text):
    try:
        if not decode_text:
            return {"success": False, "message": "No QR code provided"}
        components = decode_text.strip().split('_')
        ...
        # Step 1: Validate Product QR existence
        if not frappe.db.exists("Product QR", product_qr_name):
        ...
        qr_data_list = frappe.get_list(
            ...
            fields=[
                "name", "product_qr", "product_table_name", "product_qr_id",
                "points", "qr_code_image", "scanned", "carpenter_id",
                "redeem_date", "generated_date", "generated_time", "earned_amount","points_added","point_history_added"
            ],
            order_by="generated_date desc, generated_time desc",
            limit=1
        )
        ...
        # Step 3: Fetch product_name
        product_name = frappe.get_value("Product", {"name": qr_doc.product_table_name}, "product_name")
        if not product_name:
            return {"success": False, "message": "Product name not found"}

        # Step 4: Fetch Reward Conversion
        ...
        # Step 5: If already scanned → just return details
        if qr_doc.scanned:
            return {
                "success": True,
        ...
        frappe.log_error("QR code Scan Error",str(e))
        return {"success": False, "message": str(e)}
...
@frappe.whitelist()
def scan_and_update_qr(product_qr_id):
    try:
        if not product_qr_id:
        ...
        # ---------------------------
        # STEP 3: Identify Customer from Session
        # ---------------------------
        logged_in_user = frappe.session.user
        ...
        return {
            "success": True,
            "message": "QR Code already scanned and points added successfully.",
            "product_name": product_name,
            "product_qr_doc_name": qr_data["product_qr"],
            "product_qr_id": qr_data["name"],
            "points": qr_data["points"],
        ...
        frappe.log_error("QR Scan Failed", frappe.get_traceback())
        return {"success": False, "message": str(e)}
